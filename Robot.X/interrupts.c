...
void HighISR(void) {
  ...
  if (INTCONbits.INT0IF) {
    ...
    for (i = 0; i < 32; i++) {
      if (message[i] == '\0') break;                 // End of String
      if (message[i] == 0x33) IS_START = !IS_START;  // Code Middle Button
    }
    ...
    // Si decelerer, decroite CCPR de 10 toute les 100ms
    ...
    if (IS_START_OLD != IS_START || VOLTAGE_OLD != VOLTAGE ||
      ...
      for (i = 0; i < 3; i++) {
        if (buffer[i] != '\0') TEXTE_UART[15 + i] = buffer[i];
      }
      itoa(CCPR_CALC, buffer);  // Max = 1024 (theorical)
      for (i = 0; i < 4; i++) {
        if (buffer[i] != '\0') TEXTE_UART[26 + i] = buffer[i];
      }
      PIE1bits.TXIE = 1;
    }
